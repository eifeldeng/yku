...
if ($cmd == 'list') {
    ...
    $process = new swoole_process ( function (swoole_process $worker) use ($name, $cmd, $phpStart) {
        $worker->exec ( $phpStart, array (
                STARTBASEPATH . "/lib/YkuServer/shell/start.php",
                $cmd,
                $name 
        ) );
    }, false );
...
function StartServ($phpStart, $cmd, $name) {
    $process = new swoole_process ( function (swoole_process $worker) use ($name, $cmd, $phpStart) { // 目前指支持一个
        ...
        StartLogTimer ( __LINE__ . '   ' . $phpStart . ' ' . STARTBASEPATH . '/lib/YkuServer/shell/start.php ' . $cmd . ' ' . $name );
    }, false );
    $pid = $process->start ();
    $exeRet = swoole_process::wait ();
    if ($exeRet ['code']) { // 创建失败
 ...
 *
 */
function getServerMasterPid($name) {
    $pid = false;
...
function getServerManagerPid($name) {
    $pid = false;
