...
class TP_Template {
	...
	private $is_compile 		= false; //是否需要每次编译
	private $driver_config 		= 'Simple';
	 ...
	 * @return bool
	 */
	public function set_template_config() {
		$template_path = $this->template_path;
		$template_c_path = $this->template_c_path;
		$this->template_path = TPL_PATH.$template_path;
		$this->template_c_path = TPL_PATH.$template_c_path;
		return true;
	}
	...
	private function read_template($template_file_name) {
		if (!file_exists($template_file_name)) return false;
	...
	private function compile_template($compile_file_name, $str) {
		...
		if ($ret == false) {
			TP_Log::error("{$compile_file_name} can not write");
			return false;
		}
	}
	...
	private function check_path() {
		if (!is_dir($this->template_path) || !is_readable($this->template_path)) return false;
		if (!is_dir($this->template_c_path) || !is_readable($this->template_c_path)) return false;
		return true;
	}
	...
	private function get_driver($driver) {
		$diver_path = strtolower($driver) . '.view.php';
		if (self::$driver === NULL) {
			require_once($diver_path);
			$class = 'TP_'.$driver.'_View';
...
class TP_View extends TP_Template {
	...
	public function display($template = '') {
		...
		foreach ($this->template_arr as $file_name) {
			...
			if (!file_exists($complie_file_name)) return false;
