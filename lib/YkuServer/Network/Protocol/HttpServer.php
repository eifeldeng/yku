 ...
 */
namespace YkuServer\Network\Protocol;

use YkuServer;

class HttpServer extends YkuServer\Network\Protocol implements YkuServer\Server\Protocol
{
    ...
    {
        $this->parser = new YkuServer\Http\Parser;
    }
    ...
    function checkHeader($client_id, $http_data)
    {
        ...
        if (!isset($this->requests[$client_id])) {
            ...
            if ($ret === false) {
                ...
                $request = new YkuServer\Http\Request;
                //GET没有body
    ...
    function onReceive($serv, $client_id, $from_id, $data)
    {
        ...
        {
            $response = new YkuServer\Http\Response;
            //处理请求，产生response对象
    ...
    function afterResponse(YkuServer\Http\Request $request, YkuServer\Http\Response $response)
    {
    ...
    /**
     * 解析请求
     * @param $request YkuServer\Http\Request
     * @return null
     */
     ...
     * 发送响应
     * @param $request YkuServer\Http\Request
     * @param $response YkuServer\Http\Response
     * @return bool
     */
    function response(YkuServer\Http\Request $request, YkuServer\Http\Response $response)
    {
     ...
     * 发生了http错误
     * @param                 $code
     * @param YkuServer\Http\Response $response
     * @param string $content
     */
    function httpError($code, YkuServer\Http\Response $response, $content = '')
    {
        $response->send_http_status($code);
        $response->head['Content-Type'] = 'text/html';
        $response->body = YkuServer\Error::info(YkuServer\Http\Response::$HTTP_HEADERS[$code], "<p>$content</p><hr><address>" . self::SOFTWARE . " at {$this->server->host} Port {$this->server->port}</address>");
    }
    ...
    function onError()
    {
        ...
        if (empty($this->currentResponse)) {
            $this->currentResponse = new YkuServer\Http\Response();
        }
     ...
     * @return null
     */
    function onRequest(YkuServer\Http\Request $request)
    {
    }
